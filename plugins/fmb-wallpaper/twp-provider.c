...
static void   twp_menu_provider_init            (FmbxMenuProviderIface *iface);
static void   twp_provider_finalize             (GObject                  *object);
static GList *twp_provider_get_file_actions     (FmbxMenuProvider      *menu_provider,
                                                 GtkWidget                *window,
                                                 GList                    *files);
...
FMBX_DEFINE_TYPE_WITH_CODE (TwpProvider, twp_provider, G_TYPE_OBJECT,
                               FMBX_IMPLEMENT_INTERFACE (FMBX_TYPE_MENU_PROVIDER,
                                                            twp_menu_provider_init));

static void
twp_menu_provider_init (FmbxMenuProviderIface *iface)
{
...
static GList*
twp_provider_get_file_actions (FmbxMenuProvider *menu_provider,
                               GtkWidget           *window,
                               GList               *files)
{
  ...
  if (files->next == NULL)
    {
      ...
      if (!fmbx_file_info_is_directory (files->data))
        {
          /* get the mime type of the file */
          mime_type = fmbx_file_info_get_mime_type (files->data);

          if (g_str_has_prefix (mime_type, "image/")
              && (fmbx_file_info_has_mime_type (files->data, "image/jpeg")
                  || fmbx_file_info_has_mime_type (files->data, "image/png")
                  || fmbx_file_info_has_mime_type (files->data, "image/svg+xml")
                  || fmbx_file_info_has_mime_type (files->data, "image/svg+xml-compressed")))
            {
...
twp_action_set_wallpaper (GtkAction *action,
                          gpointer   user_data)
{
  FmbxFileInfo *file_info = user_data;
  ...
  if (desktop_type != DESKTOP_TYPE_NONE)
    {
      file_uri = fmbx_file_info_get_uri (file_info);
  ...
  switch (desktop_type)
    {
      case DESKTOP_TYPE_XFCE:
        ...
        channel = blconf_channel_get ("xfce4-desktop");
        ...
        blconf_channel_set_string (channel, image_path_prop, file_name);
        blconf_channel_set_bool (channel, image_show_prop, TRUE);

        /* If there isn't a wallpaper style set, then set one */
        current_image_style = blconf_channel_get_int (channel, image_style_prop, -1);
        if (current_image_style == -1)
          {
            blconf_channel_set_int (channel, image_style_prop, 0);
          }
         ...
         * it is) and use that. */
        is_single_workspace = blconf_channel_get_bool (channel, "/backdrop/single-workspace-mode", TRUE);
        if (is_single_workspace)
          {
            workspace = blconf_channel_get_int (channel, "/backdrop/single-workspace-number", 0);
          }
        ...
        if (current_image_style == -1)
          {
            blconf_channel_set_int (channel, image_style_prop, 5);
          }
 ...
 * the libbladeui library.
 * http://git.xfce.org/blade/libbladeui/tree/libbladeui/xfce-spawn.c#n193
 */
static gint
