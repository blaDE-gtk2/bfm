              ...
              gchar **zipfile_return)
{
  ...
  tmpdir = g_strdup ("/tmp/fmb-sendto-email.XXXXXX");
...
main (int argc, char **argv)
{
  ...
  if (G_UNLIKELY (argc == 1))
    {
      g_fprintf (stderr, "Usage: fmb-sendto-email [FILES...]\n");
      return EXIT_FAILURE;
    }
      ...
      if (parent == NULL)
        {
          ...
          g_fprintf (stderr, "fmb-sendto-email: Invalid argument \"%s\": %s\n", argv[n], error->message);
          g_error_free (error);
          g_object_unref (file);
          return EXIT_FAILURE;
        }
  ...
  if (G_LIKELY (attachments != NULL))
    {
      ...
      if (!blxo_execute_preferred_application ("MailReader", mailto->str, NULL, NULL, &error))
        {
