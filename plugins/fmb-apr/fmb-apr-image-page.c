...
static void fmb_apr_image_page_file_changed  (FmbAprAbstractPage    *abstract_page,
                                                 FmbxFileInfo          *file);
...
struct _FmbAprImagePageClass
{
  FmbAprAbstractPageClass __parent__;
};

struct _FmbAprImagePage
{
...
FMBX_DEFINE_TYPE (FmbAprImagePage,
                     fmb_apr_image_page,
...
static void
fmb_apr_image_page_class_init (FmbAprImagePageClass *klass)
{
  FmbAprAbstractPageClass *fmbapr_abstract_page_class;

  fmbapr_abstract_page_class = FMB_APR_ABSTRACT_PAGE_CLASS (klass);
  fmbapr_abstract_page_class->file_changed = fmb_apr_image_page_file_changed;
}


  
static void
fmb_apr_image_page_init (FmbAprImagePage *image_page)
{
  ...
  fmbx_property_page_set_label (FMBX_PROPERTY_PAGE (image_page), _("Image"));
  ...
  for (n = 0; n < G_N_ELEMENTS (TAIP_EXIF); ++n)
    {
      ...
      blxo_binding_new (G_OBJECT (image_page->exif_labels[n]), "visible", G_OBJECT (label), "visible");
...
static void
fmb_apr_image_page_file_changed (FmbAprAbstractPage *abstract_page,
                                    FmbxFileInfo       *file)
{
  FmbAprImagePage *image_page = FMB_APR_IMAGE_PAGE (abstract_page);
