...
GType
fmbx_preferences_provider_get_type (void)
{
  ...
  if (g_once_init_enter (&type__volatile))
    {
      type = g_type_register_static_simple (G_TYPE_INTERFACE,
                                            I_("FmbxPreferencesProvider"),
                                            sizeof (FmbxPreferencesProviderIface),
                                            NULL,
                                            0,
                                            NULL,
                                            0);
...
/**
 * fmbx_preferences_provider_get_actions:
 * @provider : a #FmbxPreferencesProvider.
 ...
 **/
GList*
fmbx_preferences_provider_get_actions (FmbxPreferencesProvider *provider,
                                          GtkWidget                  *window)
{
  GList *actions;

  g_return_val_if_fail (FMBX_IS_PREFERENCES_PROVIDER (provider), NULL);
  g_return_val_if_fail (GTK_IS_WINDOW (window), NULL);

  if (FMBX_PREFERENCES_PROVIDER_GET_IFACE (provider)->get_actions != NULL)
    {
      /* query the actions from the implementation */
      actions = (*FMBX_PREFERENCES_PROVIDER_GET_IFACE (provider)->get_actions) (provider, window);
