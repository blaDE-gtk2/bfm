...
GType
fmbx_renamer_provider_get_type (void)
{
  ...
  if (g_once_init_enter (&type__volatile))
    {
      type = g_type_register_static_simple (G_TYPE_INTERFACE,
                                            I_("FmbxRenamerProvider"),
                                            sizeof (FmbxRenamerProviderIface),
                                            NULL,
                                            0,
                                            NULL,
                                            0);
...
/**
 * fmbx_renamer_provider_get_renamers:
 * @provider : a #FmbxRenamerProvider.
 *
 * Returns the list of #FmbxRenamer<!---->s provided by the
 * specified @provider.
 *
 * The real implementation of this method MUST return the #FmbxRenamer<!---->s
 * with floating references (the default for g_object_new() on #GtkWidget
 * derived types).
 *
 * The returned #FmbxRenamer<!---->s will be reffed and sinked automatically
 * by this function.
 *
 * As a special note, this method automatically takes a reference on the
 * @provider for every #FmbxRenamer returned from the real implementation
 ...
 *
 * Return value: the list of #FmbxRenamer<!---->s provided by the
 *               specified @provider.
 **/
GList*
fmbx_renamer_provider_get_renamers (FmbxRenamerProvider *provider)
{
  GList *renamers;
  GList *lp;

  g_return_val_if_fail (FMBX_IS_RENAMER_PROVIDER (provider), NULL);

  /* determine the renamers from the real implementation */
  renamers = (*FMBX_RENAMER_PROVIDER_GET_IFACE (provider)->get_renamers) (provider);
  ...
  fmbx_object_list_take_reference (renamers, provider);
