...
static void     fmb_progress_dialog_dispose            (GObject              *object);
static void     fmb_progress_dialog_finalize           (GObject              *object);
static void     fmb_progress_dialog_shown              (FmbProgressDialog *dialog);
static gboolean fmb_progress_dialog_closed             (FmbProgressDialog *dialog);
static gboolean fmb_progress_dialog_toggled            (FmbProgressDialog *dialog, 
                                                           GdkEventButton       *button,
                                                           GtkStatusIcon        *status_icon);
static void     fmb_progress_dialog_update_status_icon (FmbProgressDialog *dialog);



struct _FmbProgressDialogClass
{
  GtkWindowClass __parent__;
};

struct _FmbProgressDialog
{
...
G_DEFINE_TYPE (FmbProgressDialog, fmb_progress_dialog, GTK_TYPE_WINDOW);



static void
fmb_progress_dialog_class_init (FmbProgressDialogClass *klass)
{
  GObjectClass *gobject_class;

  /* Determine parent type class */
  fmb_progress_dialog_parent_class = g_type_class_peek_parent (klass);

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->dispose = fmb_progress_dialog_dispose;
  gobject_class->finalize = fmb_progress_dialog_finalize;
}



static void
fmb_progress_dialog_init (FmbProgressDialog *dialog)
{
  dialog->views = NULL;
  ...
  g_signal_connect_swapped (dialog, "show", 
                            G_CALLBACK (fmb_progress_dialog_shown), dialog);

  g_signal_connect (dialog, "delete-event", 
                    G_CALLBACK (fmb_progress_dialog_closed), dialog);
...
static void
fmb_progress_dialog_dispose (GObject *object)
{
  (*G_OBJECT_CLASS (fmb_progress_dialog_parent_class)->dispose) (object);
}



static void
fmb_progress_dialog_finalize (GObject *object)
{
  FmbProgressDialog *dialog = FMB_PROGRESS_DIALOG (object);
  ...
  (*G_OBJECT_CLASS (fmb_progress_dialog_parent_class)->finalize) (object);
}



static void
fmb_progress_dialog_shown (FmbProgressDialog *dialog)
{
  _fmb_return_if_fail (FMB_IS_PROGRESS_DIALOG (dialog));

  /* show the status icon */
  if (dialog->status_icon == NULL)
    {
      dialog->status_icon = gtk_status_icon_new_from_icon_name ("stock_folder-copy");
      fmb_progress_dialog_update_status_icon (dialog);
      g_signal_connect_swapped (dialog->status_icon, "button-press-event",
                                G_CALLBACK (fmb_progress_dialog_toggled),
                                GTK_WIDGET (dialog));
    }
}
...
static gboolean
fmb_progress_dialog_closed (FmbProgressDialog *dialog)
{
  _fmb_return_val_if_fail (FMB_IS_PROGRESS_DIALOG (dialog), FALSE);
...
static gboolean
fmb_progress_dialog_toggled (FmbProgressDialog *dialog,
                                GdkEventButton       *event,
                                GtkStatusIcon        *status_icon)
{
  _fmb_return_val_if_fail (FMB_IS_PROGRESS_DIALOG (dialog), FALSE);
  _fmb_return_val_if_fail (GTK_IS_STATUS_ICON (status_icon), FALSE);
...
static void
fmb_progress_dialog_view_needs_attention (FmbProgressDialog *dialog,
                                             FmbProgressView   *view)
{
  _fmb_return_if_fail (FMB_IS_PROGRESS_DIALOG (dialog));
...
static void
fmb_progress_dialog_job_finished (FmbProgressDialog *dialog,
                                     FmbProgressView   *view)
{
  guint n_views;
...
static void
fmb_progress_dialog_update_status_icon (FmbProgressDialog *dialog)
{
  gchar *tooltip_text;
  guint  n_views;

  _fmb_return_if_fail (FMB_IS_PROGRESS_DIALOG (dialog));
  _fmb_return_if_fail (GTK_IS_STATUS_ICON (dialog->status_icon));
...
GtkWidget*
fmb_progress_dialog_new (void)
{
  return g_object_new (FMB_TYPE_PROGRESS_DIALOG, NULL);
}



void
fmb_progress_dialog_add_job (FmbProgressDialog *dialog,
                                FmbJob            *job,
                                ...
                                const gchar          *title)
{
  ...
  _fmb_return_if_fail (g_utf8_validate (title, -1, NULL));

  view = fmb_progress_view_new_with_job (job);
  fmb_progress_view_set_icon_name (FMB_PROGRESS_VIEW (view), icon_name);
  fmb_progress_view_set_title (FMB_PROGRESS_VIEW (view), title);
  ...
  g_signal_connect_swapped (view, "need-attention", 
                            G_CALLBACK (fmb_progress_dialog_view_needs_attention), dialog);

  g_signal_connect_swapped (view, "finished",
                            G_CALLBACK (fmb_progress_dialog_job_finished), dialog);
...
gboolean
fmb_progress_dialog_has_jobs (FmbProgressDialog *dialog)
{
  _fmb_return_val_if_fail (FMB_IS_PROGRESS_DIALOG (dialog), FALSE);
