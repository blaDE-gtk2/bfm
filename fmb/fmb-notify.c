...
#ifdef HAVE_LIBNOTIFY
static gboolean fmb_notify_initted = FALSE;



static gboolean
fmb_notify_init (void)
{
  gchar *spec_version = NULL;

  if (!fmb_notify_initted
      && notify_init (PACKAGE_NAME))
    {
      ...
      fmb_notify_initted = TRUE;
    }

  return fmb_notify_initted;
}



static void
fmb_notify_show (FmbDevice *device,
                    const gchar  *summary,
                    const gchar  *message)
{
  ...
  icon = fmb_device_get_icon (device);
  if (icon != NULL)
    {
  ...
  g_object_set_data_full (G_OBJECT (device), I_("fmb-notification"),
...
static gboolean
fmb_notify_device_readonly (FmbDevice *device)
{
  GFile     *mount_point;
...
void
fmb_notify_unmount (FmbDevice *device)
{
#ifdef HAVE_LIBNOTIFY
  gchar       *name;
  const gchar *summary;
  ...
  name = fmb_device_get_name (device);

  if (fmb_notify_device_readonly (device))
    {
  ...
  fmb_notify_show (device, summary, message);
...
void
fmb_notify_eject (FmbDevice *device)
{
#ifdef HAVE_LIBNOTIFY
  gchar       *name;
  const gchar *summary;
...
void
fmb_notify_finish (FmbDevice *device)
{
#ifdef HAVE_LIBNOTIFY
  NotifyNotification *notification;
  ...
  notification = g_object_get_data (G_OBJECT (device), I_("fmb-notification"));
  if (notification != NULL)
    {
      notify_notification_close (notification, NULL);
      g_object_set_data (G_OBJECT (device), I_("fmb-notification"), NULL);
    }
#endif
}
