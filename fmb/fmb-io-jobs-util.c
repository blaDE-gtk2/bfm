...
/**
 * fmb_io_jobs_util_next_duplicate_file:
 * @job   : a #FmbJob.
 ...
 **/
GFile *
fmb_io_jobs_util_next_duplicate_file (FmbJob *job,
                                         GFile     *file,
                                         gboolean   copy,
                                         guint      n,
                                         GError   **error)
{
  ...
  _fmb_return_val_if_fail (0 < n, NULL);
  _fmb_return_val_if_fail (error == NULL || *error == NULL, NULL);
  _fmb_return_val_if_fail (!fmb_g_file_is_root (file), NULL);
  ...
  info = g_file_query_info (file, G_FILE_ATTRIBUTE_STANDARD_TYPE ","
                            ...
                            blxo_job_get_cancellable (BLXO_JOB (job)), &err);
  ...
  if (copy)
    {
      ...
      if (g_file_info_get_file_type (info) != G_FILE_TYPE_DIRECTORY)
        dot = fmb_util_str_get_extension (old_display_name);
