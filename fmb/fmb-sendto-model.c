...
static void fmb_sendto_model_finalize   (GObject                *object);
static void fmb_sendto_model_load       (FmbSendtoModel      *sendto_model);
static void fmb_sendto_model_event      (GFileMonitor           *monitor,
                                            GFile                  *file,
                                            GFile                  *other_file,
...
struct _FmbSendtoModelClass
{
  GObjectClass __parent__;
};

struct _FmbSendtoModel
{
  GObject __parent__;
  GList  *monitors;
  GList  *handlers;
...
G_DEFINE_TYPE (FmbSendtoModel, fmb_sendto_model, G_TYPE_OBJECT)



static void
fmb_sendto_model_class_init (FmbSendtoModelClass *klass)
{
  GObjectClass *gobject_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize = fmb_sendto_model_finalize;
}



static void
fmb_sendto_model_init (FmbSendtoModel *sendto_model)
{
...
static void
fmb_sendto_model_finalize (GObject *object)
{
  FmbSendtoModel *sendto_model = FMB_SENDTO_MODEL (object);
  GList             *lp;
  ...
  (*G_OBJECT_CLASS (fmb_sendto_model_parent_class)->finalize) (object);
}
...
static void
fmb_sendto_model_load (FmbSendtoModel *sendto_model)
{
  ...
  specs = xfce_resource_match (XFCE_RESOURCE_DATA, "Fmb/sendto/*.desktop", TRUE);
...
static void
fmb_sendto_model_event (GFileMonitor     *monitor,
                           GFile            *file,
                           GFile            *other_file,
                           GFileMonitorEvent event_type,
                           gpointer          user_data)
{
  FmbSendtoModel *sendto_model = FMB_SENDTO_MODEL (user_data);
  ...
  fmb_sendto_model_load (sendto_model);
}



/**
 * fmb_sendto_model_get_default:
 *
 * Returns a reference to the default #FmbSendtoModel
 ...
 **/
FmbSendtoModel*
fmb_sendto_model_get_default (void)
{
  static FmbSendtoModel *sendto_model = NULL;

  if (G_UNLIKELY (sendto_model == NULL))
    {
      sendto_model = g_object_new (FMB_TYPE_SENDTO_MODEL, NULL);
...
/**
 * fmb_sendto_model_get_matching:
 * @sendto_model : a #FmbSendtoModel.
 * @files        : a #GList of #FmbFile<!---->s.
 * 
 ...
 **/
GList*
fmb_sendto_model_get_matching (FmbSendtoModel *sendto_model,
                                  GList             *files)
{
  ...
  _fmb_return_val_if_fail (FMB_IS_SENDTO_MODEL (sendto_model), NULL);
      ...
      for (n = 0; datadirs[n] != NULL; ++n)
        {
          ...
          dir = g_build_filename (datadirs[n], "Fmb", "sendto", NULL);
          ...
          if (G_LIKELY (monitor != NULL))
            {
              g_signal_connect (monitor, "changed", G_CALLBACK (fmb_sendto_model_event), sendto_model);
          ...
          for (fp = files; fp != NULL; fp = fp->next)
            {
              ...
              for (n = 0; mime_types[n] != NULL; ++n)
                {
                  content_type = fmb_file_get_content_type (fp->data);
