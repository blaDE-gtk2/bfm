...
GFile *
fmb_g_file_new_for_desktop (void)
{
...
GFile *
fmb_g_file_new_for_bookmarks (void)
{
  gchar *filename;
  GFile *bookmarks;
...
gboolean
fmb_g_file_is_trashed (GFile *file)
{
...
gboolean
fmb_g_file_is_trash (GFile *file)
{
  char *uri;
...
GKeyFile *
fmb_g_file_query_key_file (GFile              *file,
                              ...
                              GError            **error)
{
  ...
  _fmb_return_val_if_fail (cancellable == NULL || G_IS_CANCELLABLE (cancellable), NULL);
...
gboolean
fmb_g_file_write_key_file (GFile        *file,
                              GKeyFile     *key_file,
                              GCancellable *cancellable,
                              GError      **error)
{
  ...
  _fmb_return_val_if_fail (G_IS_FILE (file), FALSE);
  _fmb_return_val_if_fail (key_file != NULL, FALSE);
...
gchar *
fmb_g_file_get_location (GFile *file)
{
  gchar *location;
...
gchar *
fmb_g_file_get_display_name (GFile *file)
{
  gchar *base_name;
  gchar *display_name;
  ...
  if (G_LIKELY (base_name != NULL))
    {
      ...
      else if (fmb_g_file_is_trash (file))
...
gchar *
fmb_g_file_get_display_name_remote (GFile *mount_point)
{
  gchar       *scheme;
  gchar       *parse_name;
  const gchar *p;
  ...
  _fmb_return_val_if_fail (G_IS_FILE (mount_point), NULL);
  ...
  if (display_name == NULL)
    display_name = fmb_g_file_get_display_name (mount_point);
...
gboolean
fmb_g_vfs_is_uri_scheme_supported (const gchar *scheme)
{
  ...
  _fmb_return_val_if_fail (scheme != NULL && *scheme != '\0', FALSE);
 ...
 **/
gboolean
fmb_g_file_get_free_space (GFile   *file,
...
gchar *
fmb_g_file_get_free_space_string (GFile *file, gboolean file_size_binary)
{
  gchar             *fs_free_str;
  gchar             *fs_size_str;
  guint64            fs_free;
  ...
  if (fmb_g_file_get_free_space (file, &fs_free, &fs_size)
      && fs_size > 0)
    {
...
GType
fmb_g_file_list_get_type (void)
{
  static GType type = G_TYPE_INVALID;

  if (G_UNLIKELY (type == G_TYPE_INVALID))
    {
      type = g_boxed_type_register_static (I_("FmbGFileList"),
                                           (GBoxedCopyFunc) fmb_g_file_list_copy,
                                           (GBoxedFreeFunc) fmb_g_file_list_free);
    }
...
/**
 * fmb_g_file_list_new_from_string:
 ...
 **/
GList *
fmb_g_file_list_new_from_string (const gchar *string)
{
  GList  *list = NULL;
  gchar **uris;
  gsize   n;
 ...
 **/
gchar **
fmb_g_file_list_to_stringv (GList *list)
{
  gchar **uris;
  guint   n;
  GList  *lp;
...
gboolean
fmb_g_app_info_launch (GAppInfo          *info,
                          ...
                          GError           **error)
{
  ...
  _fmb_return_val_if_fail (G_IS_APP_INFO (info), FALSE);
  _fmb_return_val_if_fail (working_directory == NULL || G_IS_FILE (working_directory), FALSE);
  _fmb_return_val_if_fail (path_list != NULL, FALSE);
  _fmb_return_val_if_fail (G_IS_APP_LAUNCH_CONTEXT (context), FALSE);
  _fmb_return_val_if_fail (error == NULL || *error == NULL, FALSE);
  ...
  if (working_directory != NULL)
    {
      ...
      if (new_path != NULL)
        {
          /* switch to the desired working directory, remember that of Fmb itself */
          old_path = fmb_util_change_working_directory (new_path);
  ...
  /* check if we need to reset the working directory to the one Fmb was
   * opened from */
  if (old_path != NULL)
    {
      /* switch to Fmb's original working directory */
      new_path = fmb_util_change_working_directory (old_path);
...
gboolean
fmb_g_app_info_should_show (GAppInfo *info)
{
#ifdef HAVE_GIO_UNIX
  _fmb_return_val_if_fail (G_IS_APP_INFO (info), FALSE);
