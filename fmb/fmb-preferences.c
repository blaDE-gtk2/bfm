...
static void     fmb_preferences_finalize           (GObject                *object);
static void     fmb_preferences_get_property       (GObject                *object,
                                                       guint                   prop_id,
                                                       GValue                 *value,
                                                       GParamSpec             *pspec);
static void     fmb_preferences_set_property       (GObject                *object,
                                                       guint                   prop_id,
                                                       const GValue           *value,
                                                       GParamSpec             *pspec);
static void     fmb_preferences_prop_changed       (BlconfChannel          *channel,
                                                       const gchar            *prop_name,
                                                       const GValue           *value,
                                                       FmbPreferences      *preferences);
static void     fmb_preferences_load_rc_file       (FmbPreferences      *preferences);



struct _FmbPreferencesClass
{
  GObjectClass __parent__;
};

struct _FmbPreferences
{
  GObject __parent__;
...
/* don't do anything in case blconf_init() failed */
static gboolean no_blconf = FALSE;



G_DEFINE_TYPE (FmbPreferences, fmb_preferences, G_TYPE_OBJECT)
...
static void
fmb_preferences_class_init (FmbPreferencesClass *klass)
{
  GObjectClass *gobject_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize = fmb_preferences_finalize;
  gobject_class->get_property = fmb_preferences_get_property;
  gobject_class->set_property = fmb_preferences_set_property;

  /**
   * FmbPreferences:default-view:
   *
   * The name of the widget class, which should be used for the
   * view pane in new #FmbWindow<!---->s or "void" to use the
  ...
  /**
   * FmbPreferences:hidden-bookmarks:
   *
  ...
  /**
   * FmbPreferences:last-compact-view-zoom-level:
   *
   * The last selected #FmbZoomLevel for the #FmbCompactView.
   **/
  ...
  /**
   * FmbPreferences:last-details-view-column-order:
   *
   * The comma separated list of columns that specifies the order of the
   * columns in the #FmbDetailsView.
   **/
  preferences_props[PROP_LAST_DETAILS_VIEW_COLUMN_ORDER] =
      g_param_spec_string ("last-details-view-column-order",
                           ...
                           NULL,
                           "FMB_COLUMN_NAME,FMB_COLUMN_SIZE,FMB_COLUMN_TYPE,FMB_COLUMN_DATE_MODIFIED",
                           BLXO_PARAM_READWRITE);

  /**
   * FmbPreferences:last-details-view-column-widths:
   *
  ...
  /**
   * FmbPreferences:last-details-view-fixed-columns:
   *
   * %TRUE to use fixed column widths in the #FmbDetailsView. Else the
  ...
  /**
   * FmbPreferences:last-details-view-visible-columns:
   *
   * The comma separated list of visible columns in the #FmbDetailsView.
   **/
  preferences_props[PROP_LAST_DETAILS_VIEW_VISIBLE_COLUMNS] =
      g_param_spec_string ("last-details-view-visible-columns",
                           ...
                           NULL,
                           "FMB_COLUMN_DATE_MODIFIED,FMB_COLUMN_NAME,FMB_COLUMN_SIZE,FMB_COLUMN_TYPE",
                           BLXO_PARAM_READWRITE);

  /**
   * FmbPreferences:last-details-view-zoom-level:
   *
   * The last selected #FmbZoomLevel for the #FmbDetailsView.
   **/
  ...
  /**
   * FmbPreferences:last-icon-view-zoom-level:
   *
   * The last selected #FmbZoomLevel for the #FmbIconView.
   **/
  ...
  /**
   * FmbPreferences:last-location-bar:
   *
   * The name of the widget class, which should be used for the
   * location bar in #FmbWindow<!---->s or "void" to hide the
   * location bar.
   **/
  ...
  /**
   * FmbPreferences:last-menubar-visible:
   *
  ...
  /**
   * FmbPreferences:last-separator-position:
   *
  ...
  /**
   * FmbPreferences:last-show-hidden:
   *
  ...
  /**
   * FmbPreferences:last-side-pane:
   *
   * The name of the widget class, which should be used for the
   * side pane in #FmbWindow<!---->s or "void" to hide the
  ...
  /**
   * FmbPreferences:last-sort-column:
   *
  ...
  /**
   * FmbPreferences:last-sort-order:
   *
  ...
  preferences_props[PROP_LAST_SORT_ORDER] =
      g_param_spec_enum ("last-sort-order",
                         ...
                         BLXO_PARAM_READWRITE);
  /**
   * FmbPreferences:last-statusbar-visible:
   *
  ...
  /**
   * FmbPreferences:last-view:
   *
   * The name of the widget class, which should be used for the
   * main view component in #FmbWindow<!---->s.
   **/
  ...
  /**
   * FmbPreferences:last-window-height:
   *
   * The last known height of a #FmbWindow, which will be used as
  ...
  /**
   * FmbPreferences:last-window-width:
   *
   * The last known width of a #FmbWindow, which will be used as
  ...
  /**
   * FmbPreferences:last-window-maximized:
   *
   * The last known maximized state of a #FmbWindow, which will be used as
   * default width for newly created windows.
   **/
  ...
  /**
   * FmbPreferences:misc-always-show-tabs:
   *
  ...
  /**
   * FmbPreferences:misc-volume-management:
   *
  ...
  /**
   * FmbPreferences:misc-case-sensitive:
   *
  ...
  /**
   * FmbPreferences:misc-date-style:
   *
  ...
  /**
   * FmbPreferences:misc-execute-shell-scripts-by-default:
   *
  ...
  /**
   * FmbPreferences:misc-folders-first:
   *
  ...
  /**
   * FmbPreferences:misc-full-path-in-title:
   *
  ...
  /**
   * FmbPreferences:misc-horizontal-wheel-navigates:
   *
   * Whether the horizontal mouse wheel is used to navigate
   * forth and back within a Fmb view.
   **/
  ...
  /**
   * FmbPreferences:misc-image-size-in-statusbar:
   *
  ...
  /**
   * FmbPreferences:misc-middle-click-in-tab:
   *
  ...
  /**
   * FmbPreferences:misc-recursive-permissions:
   *
  ...
  preferences_props[PROP_MISC_RECURSIVE_PERMISSIONS] =
      g_param_spec_enum ("misc-recursive-permissions",
                         ...
                         NULL,
                         FMB_TYPE_RECURSIVE_PERMISSIONS,
                         FMB_RECURSIVE_PERMISSIONS_ASK,
                         BLXO_PARAM_READWRITE);

  /**
   * FmbPreferences:misc-remember-geometry:
   *
   * Whether Fmb should remember the size of windows and apply
  ...
  /**
   * FmbPreferences:misc-show-about-templates:
   *
  ...
  /**
   * FmbPreferences:misc-single-click:
   *
  ...
  /**
   * FmbPreferences:misc-single-click-timeout:
   *
  ...
  /**
   * FmbPreferences:misc-small-toolbar-icons:
   *
  ...
  /**
   * FmbPreferences:misc-tab-close-middle-click:
   *
  ...
  /**
   * FmbPreferences:misc-text-beside-icons:
   *
  ...
  /**
   * FmbPreferences:misc-thumbnail-mode:
   *
  ...
  /**
   * FmbPreferences:misc-file-size-binary:
   *
  ...
  /**
   * FmbPreferences:shortcuts-icon-emblems:
   *
  ...
  /**
   * FmbPreferences:shortcuts-icon-size:
   *
  ...
  preferences_props[PROP_SHORTCUTS_ICON_SIZE] =
      g_param_spec_enum ("shortcuts-icon-size",
                         ...
                         NULL,
                         FMB_TYPE_ICON_SIZE,
                         FMB_ICON_SIZE_SMALLER,
                         BLXO_PARAM_READWRITE);

  /**
   * FmbPreferences:tree-icon-emblems:
   *
   * Whether to display emblems for file icons (if defined) in the
   * tree side pane.
   **/
...
static void
fmb_preferences_init (FmbPreferences *preferences)
{
  const gchar check_prop[] = "/last-view";

  /* don't set a channel if blconf init failed */
  if (no_blconf)
    return;

  /* load the channel */
  preferences->channel = blconf_channel_get ("fmb");

  /* check one of the property to see if there are values */
  if (!blconf_channel_has_property (preferences->channel, check_prop))
    {
      /* try to load the old config file */
      fmb_preferences_load_rc_file (preferences);

      /* set the string we check */
      if (!blconf_channel_has_property (preferences->channel, check_prop))
        blconf_channel_set_string (preferences->channel, check_prop, "FmbIconView");
    }

  preferences->property_changed_id =
    g_signal_connect (G_OBJECT (preferences->channel), "property-changed",
                      G_CALLBACK (fmb_preferences_prop_changed), preferences);
}



static void
fmb_preferences_finalize (GObject *object)
{
  FmbPreferences *preferences = FMB_PREFERENCES (object);

  /* disconnect from the updates */
  g_signal_handler_disconnect (preferences->channel, preferences->property_changed_id);

  (*G_OBJECT_CLASS (fmb_preferences_parent_class)->finalize) (object);
}



static void
fmb_preferences_get_property (GObject    *object,
                                 guint       prop_id,
                                 GValue     *value,
                                 GParamSpec *pspec)
{
  ...
  if (G_VALUE_TYPE (value) == G_TYPE_STRV)
    {
      ...
      array = blconf_channel_get_string_list (preferences->channel, prop_name);
      g_value_take_boxed (value, array);
    }
  else if (blconf_channel_get_property (preferences->channel, prop_name, &src))
    {
      if (G_VALUE_TYPE (value) == G_VALUE_TYPE (&src))
        g_value_copy (&src, value);
      else if (!g_value_transform (&src, value))
        g_printerr ("Fmb: Failed to transform property %s\n", prop_name);
      g_value_unset (&src);
    }
  else
    {
...
static void
fmb_preferences_set_property (GObject      *object,
                                 guint         prop_id,
                                 const GValue *value,
                                 GParamSpec   *pspec)
{
  ...
  if (G_VALUE_HOLDS_ENUM (value))
    {
      ...
      if (g_value_transform (value, &dst))
        blconf_channel_set_property (preferences->channel, prop_name, &dst);
      g_value_unset (&dst);
    }
  else if (G_VALUE_HOLDS (value, G_TYPE_STRV))
    {
      /* convert to a GValue GPtrArray in blconf */
      array = g_value_get_boxed (value);
      if (array != NULL && *array != NULL)
        blconf_channel_set_string_list (preferences->channel, prop_name, (const gchar * const *) array);
      else
        blconf_channel_reset_property (preferences->channel, prop_name, FALSE);
    }
  else
    {
      /* other types we support directly */
      blconf_channel_set_property (preferences->channel, prop_name, value);
    }
...
static void
fmb_preferences_prop_changed (BlconfChannel     *channel,
                                 const gchar       *prop_name,
                                 const GValue      *value,
                                 FmbPreferences *preferences)
{
...
static void
fmb_preferences_load_rc_file (FmbPreferences *preferences)
{
  GParamSpec  **pspecs;
  GParamSpec   *pspec;
  XfceRc       *rc;
  ...
  /* find file */
  filename = xfce_resource_lookup (XFCE_RESOURCE_CONFIG, "Fmb/fmbrc");
  ...
  for (n = 0; n < nspecs; ++n)
    {
      ...
      /* blconf property name, continue if exists */
      g_snprintf (prop_name, sizeof (prop_name), "/%s", g_param_spec_get_name (pspec));
      if (blconf_channel_has_property (preferences->channel, prop_name))
        continue;
      ...
      if (G_IS_PARAM_SPEC_STRING (pspec) || G_IS_PARAM_SPEC_ENUM (pspec))
        {
          blconf_channel_set_property (preferences->channel, prop_name, &src);
        }
  ...
  if (!xfce_rc_read_bool_entry (rc, "MiscShowThumbnails", TRUE))
    {
      blconf_channel_set_string (preferences->channel,
                                 "/misc-thumbnail-mode",
  ...
  g_print ("\n\n"
           "Your Fmb settings have been migrated to Blconf.\n"
           "The config file \"%s\"\n"
           "is not used anymore.\n\n", filename);
 ...
 * fmb_preferences_get:
 *
 * Queries the global #FmbPreferences instance, which is shared
 * by all modules. The function automatically takes a reference
 * for the caller, so you'll need to call g_object_unref() when
 * you're done with it.
 *
 * Return value: the global #FmbPreferences instance.
 **/
FmbPreferences*
fmb_preferences_get (void)
{
  static FmbPreferences *preferences = NULL;

  if (G_UNLIKELY (preferences == NULL))
    {
      preferences = g_object_new (FMB_TYPE_PREFERENCES, NULL);
...
void
fmb_preferences_blconf_init_failed (void)
{
  no_blconf = TRUE;
}
