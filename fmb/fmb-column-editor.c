...
static void fmb_column_editor_finalize           (GObject                  *object);
static void fmb_column_editor_help_clicked       (GtkWidget                *button,
                                                     FmbColumnEditor       *column_editor);
static void fmb_column_editor_move_down          (GtkWidget                *button,
                                                     FmbColumnEditor       *column_editor);
static void fmb_column_editor_move_up            (GtkWidget                *button,
                                                     FmbColumnEditor       *column_editor);
static void fmb_column_editor_toggled            (GtkCellRendererToggle    *cell_renderer,
                                                     const gchar              *path_string,
                                                     FmbColumnEditor       *column_editor);
static void fmb_column_editor_toggle_visibility  (GtkWidget                *button,
                                                     FmbColumnEditor       *column_editor);
static void fmb_column_editor_update_buttons     (FmbColumnEditor       *column_editor);
static void fmb_column_editor_use_defaults       (GtkWidget                *button,
                                                     FmbColumnEditor       *column_editor);



struct _FmbColumnEditorClass
{
  FmbAbstractDialogClass __parent__;
};

struct _FmbColumnEditor
{
...
G_DEFINE_TYPE (FmbColumnEditor, fmb_column_editor, FMB_TYPE_ABSTRACT_DIALOG)



static void
fmb_column_editor_class_init (FmbColumnEditorClass *klass)
{
  GObjectClass *gobject_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize = fmb_column_editor_finalize;
}



static void
fmb_column_editor_init (FmbColumnEditor *column_editor)
{
  ...
  column_editor->preferences = fmb_preferences_get ();

  /* grab a reference on the shared column model */
  column_editor->column_model = fmb_column_model_get_default ();
  g_signal_connect_data (G_OBJECT (column_editor->column_model), "row-changed", G_CALLBACK (fmb_column_editor_update_buttons),
                         column_editor, NULL, G_CONNECT_AFTER | G_CONNECT_SWAPPED);
  g_signal_connect_data (G_OBJECT (column_editor->column_model), "rows-reordered", G_CALLBACK (fmb_column_editor_update_buttons),
                         column_editor, NULL, G_CONNECT_AFTER | G_CONNECT_SWAPPED);
  ...
  g_signal_connect (G_OBJECT (button), "clicked", G_CALLBACK (fmb_column_editor_help_clicked), column_editor);
  ...
  g_signal_connect (G_OBJECT (renderer), "toggled", G_CALLBACK (fmb_column_editor_toggled), column_editor);
  gtk_tree_view_column_pack_start (column, renderer, FALSE);
  gtk_tree_view_column_set_attributes (column, renderer,
                                       "active", FMB_COLUMN_MODEL_COLUMN_VISIBLE,
                                       "activatable", FMB_COLUMN_MODEL_COLUMN_MUTABLE,
                                       NULL);
  ...
  gtk_tree_view_column_set_attributes (column, renderer,
                                       "text", FMB_COLUMN_MODEL_COLUMN_NAME,
                                       NULL);
  gtk_tree_view_append_column (GTK_TREE_VIEW (column_editor->tree_view), column);

  /* create the "Move Up" button */
  column_editor->up_button = gtk_button_new_with_mnemonic (_("Move _Up"));
  g_signal_connect (G_OBJECT (column_editor->up_button), "clicked", G_CALLBACK (fmb_column_editor_move_up), column_editor);
  ...
  g_signal_connect (G_OBJECT (column_editor->down_button), "clicked", G_CALLBACK (fmb_column_editor_move_down), column_editor);
  ...
  g_signal_connect (G_OBJECT (column_editor->show_button), "clicked", G_CALLBACK (fmb_column_editor_toggle_visibility), column_editor);
  ...
  g_signal_connect (G_OBJECT (column_editor->hide_button), "clicked", G_CALLBACK (fmb_column_editor_toggle_visibility), column_editor);
  ...
  g_signal_connect (G_OBJECT (button), "clicked", G_CALLBACK (fmb_column_editor_use_defaults), column_editor);
  ...
  blxo_mutual_binding_new_with_negation (G_OBJECT (column_editor->preferences), "last-details-view-fixed-columns", G_OBJECT (button), "active");
  gtk_table_attach (GTK_TABLE (table), button, 0, 1, 1, 2, GTK_FILL, GTK_FILL, 0, 0);
  fmb_gtk_label_set_a11y_relation (GTK_LABEL (label), button);
  gtk_widget_show (button);

  /* setup the tree selection */
  selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (column_editor->tree_view));
  g_signal_connect_swapped (G_OBJECT (selection), "changed", G_CALLBACK (fmb_column_editor_update_buttons), column_editor);
...
static void
fmb_column_editor_finalize (GObject *object)
{
  FmbColumnEditor *column_editor = FMB_COLUMN_EDITOR (object);
  ...
  (*G_OBJECT_CLASS (fmb_column_editor_parent_class)->finalize) (object);
}



static void
fmb_column_editor_help_clicked (GtkWidget          *button,
                                   FmbColumnEditor *column_editor)
{
  _fmb_return_if_fail (FMB_IS_COLUMN_EDITOR (column_editor));
...
static void
fmb_column_editor_move_down (GtkWidget          *button,
                                FmbColumnEditor *column_editor)
{
  GtkTreeSelection *selection;
  GtkTreeModel     *model;
  ...
  if (gtk_tree_selection_get_selected (selection, &model, &iter1))
    {
      ...
      if (gtk_tree_model_iter_next (model, &iter2))
        fmb_column_model_exchange (FMB_COLUMN_MODEL (model), &iter1, &iter2);
    }
}
...
static void
fmb_column_editor_move_up (GtkWidget          *button,
                              FmbColumnEditor *column_editor)
{
  GtkTreeSelection *selection;
  GtkTreeModel     *model;
...
static void
fmb_column_editor_toggled (GtkCellRendererToggle *cell_renderer,
                              const gchar           *path_string,
                              FmbColumnEditor    *column_editor)
{
  FmbColumn column;
  ...
  _fmb_return_if_fail (GTK_IS_CELL_RENDERER_TOGGLE (cell_renderer));
  _fmb_return_if_fail (FMB_IS_COLUMN_EDITOR (column_editor));
  _fmb_return_if_fail (path_string != NULL);
  ...
  if (gtk_tree_model_get_iter (GTK_TREE_MODEL (column_editor->column_model), &iter, path))
    {
      ...
      column = fmb_column_model_get_column_for_iter (column_editor->column_model, &iter);

      /* ...determine the existing visbility setting... */
      visible = fmb_column_model_get_column_visible (column_editor->column_model, column);

      /* ...and change the visibility of the column */
      fmb_column_model_set_column_visible (column_editor->column_model, column, !visible);
    }
  gtk_tree_path_free (path);
}
...
static void
fmb_column_editor_toggle_visibility (GtkWidget          *button,
                                        FmbColumnEditor *column_editor)
{
  GtkTreeSelection *selection;
  FmbColumn      column;
...
static void
fmb_column_editor_update_buttons (FmbColumnEditor *column_editor)
{
  GtkTreeSelection *selection;
  GtkTreeModel     *model;
  GtkTreePath      *path;
  ...
  if (gtk_tree_selection_get_selected (selection, &model, &iter))
    {
      ...
      gtk_tree_model_get (model, &iter, FMB_COLUMN_MODEL_COLUMN_MUTABLE, &mutable, FMB_COLUMN_MODEL_COLUMN_VISIBLE, &visible, -1);
...
static void
fmb_column_editor_use_defaults (GtkWidget          *button,
                                   FmbColumnEditor *column_editor)
{
 ...
 *
 * Convenience function to display a #FmbColumnEditor.
 *
 ...
 **/
void
fmb_show_column_editor (gpointer parent)
{
  ...
  dialog = g_object_new (FMB_TYPE_COLUMN_EDITOR, NULL);
