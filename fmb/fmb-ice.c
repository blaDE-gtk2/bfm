...
#ifdef HAVE_LIBSM
static void
fmb_ice_error_handler (IceConn connection)
{
  /*
...
static gboolean
fmb_ice_process_messages (GIOChannel  *channel,
...
static void
fmb_ice_connection_watch (IceConn     connection,
  ...
  if (G_LIKELY (opening))
    {
      ...
      watch_id = g_io_add_watch (channel, G_IO_ERR | G_IO_HUP | G_IO_IN, fmb_ice_process_messages, connection);
  ...
  if (G_LIKELY (!initialized))
    {
      ...
      IceSetIOErrorHandler (fmb_ice_error_handler);

      /* add the connection watch */
      IceAddConnectionWatch (fmb_ice_connection_watch, NULL);
