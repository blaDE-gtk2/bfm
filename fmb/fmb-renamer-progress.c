...
static void     fmb_renamer_progress_finalize          (GObject                    *object);
static void     fmb_renamer_progress_destroy           (GtkObject                  *object);
static gboolean fmb_renamer_progress_next_idle         (gpointer                    user_data);
static void     fmb_renamer_progress_next_idle_destroy (gpointer                    user_data);



struct _FmbRenamerProgressClass
{
  GtkAlignmentClass __parent__;
};

struct _FmbRenamerProgress
{
...
G_DEFINE_TYPE (FmbRenamerProgress, fmb_renamer_progress, GTK_TYPE_ALIGNMENT)



static void
fmb_renamer_progress_class_init (FmbRenamerProgressClass *klass)
{
  GtkObjectClass *gtkobject_class;
  GObjectClass   *gobject_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize = fmb_renamer_progress_finalize;

  gtkobject_class = GTK_OBJECT_CLASS (klass);
  gtkobject_class->destroy = fmb_renamer_progress_destroy;
}



static void
fmb_renamer_progress_init (FmbRenamerProgress *renamer_progress)
{
...
static void
fmb_renamer_progress_finalize (GObject *object)
{
  FmbRenamerProgress *renamer_progress = FMB_RENAMER_PROGRESS (object);

  /* make sure we're not finalized while the main loop is active */
  _fmb_assert (renamer_progress->next_idle_id == 0);
  _fmb_assert (renamer_progress->next_idle_loop == NULL);

  /* release the pairs */
  fmb_renamer_pair_list_free (renamer_progress->pairs_done);
  fmb_renamer_pair_list_free (renamer_progress->pairs_todo);

  (*G_OBJECT_CLASS (fmb_renamer_progress_parent_class)->finalize) (object);
}



static void
fmb_renamer_progress_destroy (GtkObject *object)
{
  FmbRenamerProgress *renamer_progress = FMB_RENAMER_PROGRESS (object);

  /* exit the internal main loop on destroy */
  fmb_renamer_progress_cancel (renamer_progress);

  (*GTK_OBJECT_CLASS (fmb_renamer_progress_parent_class)->destroy) (object);
}



static gboolean
fmb_renamer_progress_next_idle (gpointer user_data)
{
  FmbRenamerProgress *renamer_progress = FMB_RENAMER_PROGRESS (user_data);
  FmbRenamerPair     *pair;
  GtkWindow             *toplevel;
  GtkWidget             *message;
  GError                *error = NULL;
  ...
  if (G_LIKELY (renamer_progress->pairs_todo != NULL))
    {
      ...
      _fmb_assert (g_list_length (renamer_progress->pairs_todo) == renamer_progress->n_pairs_todo);
      ...
      oldname = g_strdup (fmb_file_get_display_name (pair->file));

      /* try to rename the file */
      if (!fmb_file_rename (pair->file, pair->name, NULL, FALSE, &error))
        {
      ...
      else
        {
          ...
          _fmb_assert (g_list_length (renamer_progress->pairs_done) == renamer_progress->n_pairs_done);
        }
    }
...
static void
fmb_renamer_progress_next_idle_destroy (gpointer user_data)
{
  FMB_RENAMER_PROGRESS (user_data)->next_idle_id = 0;
}



/**
 * fmb_renamer_progress_new:
 *
 * Allocates a new #FmbRenamerProgress instance.
 *
 * Return value: the newly allocated #FmbRenamerProgress.
 **/
GtkWidget*
fmb_renamer_progress_new (void)
{
  return g_object_new (FMB_TYPE_RENAMER_PROGRESS, NULL);
}



/**
 * fmb_renamer_progress_cancel:
 * @renamer_progress : a #FmbRenamerProgress.
 *
 * Cancels any pending rename operation for @renamer_progress.
 **/
void
fmb_renamer_progress_cancel (FmbRenamerProgress *renamer_progress)
{
  _fmb_return_if_fail (FMB_IS_RENAMER_PROGRESS (renamer_progress));
 ...
 **/
gboolean
fmb_renamer_progress_running (FmbRenamerProgress *renamer_progress)
{
  _fmb_return_val_if_fail (FMB_IS_RENAMER_PROGRESS (renamer_progress), FALSE);
...
/**
 * fmb_renamer_progress_run:
 * @renamer_progress : a #FmbRenamerProgress.
 * @pair_list        : a #GList of #FmbRenamePair<!---->s.
 *
 * Renames all #FmbRenamePair<!---->s in the specified @pair_list
 ...
 **/
void
fmb_renamer_progress_run (FmbRenamerProgress *renamer_progress,
                             GList                 *pairs)
{
  _fmb_return_if_fail (FMB_IS_RENAMER_PROGRESS (renamer_progress));
  ...
  fmb_renamer_pair_list_free (renamer_progress->pairs_todo);
  renamer_progress->pairs_todo = fmb_renamer_pair_list_copy (pairs);
  renamer_progress->n_pairs_todo = g_list_length (renamer_progress->pairs_todo);

  /* schedule the idle source */
  renamer_progress->next_idle_id = g_idle_add_full (G_PRIORITY_LOW, fmb_renamer_progress_next_idle,
                                                    renamer_progress, fmb_renamer_progress_next_idle_destroy);
