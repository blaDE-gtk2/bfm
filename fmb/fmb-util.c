 ...
 **/
static inline gchar *
fmb_util_strrchr_offset (const gchar *str,
                            const gchar *offset,
                            gchar        c)
{
  const gchar *p;
...
**/
gchar *
fmb_util_str_get_extension (const gchar *filename)
{
      ...
      if (strcasecmp (ext, compressed[i]) == 0)
        {
          ...
          dot2 = fmb_util_strrchr_offset (filename, dot - 1, '.');
          if (dot2 != NULL
              && dot2 != filename)
            {
...
void
fmb_util_load_bookmarks (GFile               *bookmarks_file,
                            ...
                            gpointer             user_data)
{
  ...
  _fmb_return_if_fail (G_IS_FILE (bookmarks_file));
  _fmb_return_if_fail (g_file_is_native (bookmarks_file));
  _fmb_return_if_fail (foreach_func != NULL);
 ...
 **/
gchar *
fmb_util_expand_filename (const gchar  *filename,
...
/**
 * fmb_util_humanize_file_time:
 * @file_time   : a #guint64 timestamp.
 * @date_format : the #FmbDateFormat used to humanize the @file_time.
 *
 ...
 **/
gchar*
fmb_util_humanize_file_time (guint64         file_time,
  ...
  if (G_LIKELY (file_time != 0))
    {
      ...
      if (date_style == FMB_DATE_STYLE_SIMPLE || date_style == FMB_DATE_STYLE_SHORT)
        {
          ...
          if (diff == 0)
            {
              if (date_style == FMB_DATE_STYLE_SIMPLE)
                {
                  /* TRANSLATORS: file was modified less than one day ago */
                  return g_strdup (_("Today"));
                }
              else /* if (date_style == FMB_DATE_STYLE_SHORT) */
                {
                  /* TRANSLATORS: file was modified less than one day ago */
                  return blxo_strdup_strftime (_("Today at %X"), tfile);
                }
            }
          else if (diff == 1)
            {
              if (date_style == FMB_DATE_STYLE_SIMPLE)
                ...
                {
                  /* TRANSLATORS: file was modified less than two days ago */
                  return blxo_strdup_strftime (_("Yesterday at %X"), tfile);
                }
            }
          else
            {
              if (diff > 1 && diff < 7)
                {
                  /* Days from last week */
                  date_format = (date_style == FMB_DATE_STYLE_SIMPLE) ? "%A" : _("%A at %X");
                }
              else
                {
                  /* Any other date */
                  date_format = (date_style == FMB_DATE_STYLE_SIMPLE) ? "%x" : _("%x at %X");
                }

              /* format the date string accordingly */
              return blxo_strdup_strftime (date_format, tfile);
            }
        }
      else if (date_style == FMB_DATE_STYLE_LONG)
        {
          /* use long, date(1)-like format string */
          return blxo_strdup_strftime ("%c", tfile);
        }
      else /* if (date_style == FMB_DATE_STYLE_ISO) */
        {
          /* use ISO date formatting */
          return blxo_strdup_strftime ("%Y-%m-%d %H:%M:%S", tfile);
        }
    }
 ...
 **/
GdkScreen*
fmb_util_parse_parent (gpointer    parent,
 ...
 **/
time_t
fmb_util_time_from_rfc3339 (const gchar *date_string)
{
  struct tm tm;
...
gchar *
fmb_util_change_working_directory (const gchar *new_directory)
{
  gchar *old_directory;

  _fmb_return_val_if_fail (new_directory != NULL && *new_directory != '\0', NULL);
