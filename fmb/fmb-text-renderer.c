...
static void             fmb_text_renderer_finalize                         (GObject                 *object);
static void             fmb_text_renderer_get_property                     (GObject                 *object,
                                                                               guint                    prop_id,
                                                                               GValue                  *value,
                                                                               GParamSpec              *pspec);
static void             fmb_text_renderer_set_property                     (GObject                 *object,
                                                                               guint                    prop_id,
                                                                               const GValue            *value,
                                                                               GParamSpec              *pspec);
static void             fmb_text_renderer_get_size                         (GtkCellRenderer         *renderer,
                                                                               GtkWidget               *widget,
                                                                               ...
                                                                               gint                    *width,
                                                                               gint                    *height);
static void             fmb_text_renderer_render                           (GtkCellRenderer         *renderer,
                                                                               GdkWindow               *window,
                                                                               GtkWidget               *widget,
...
static GtkCellEditable *fmb_text_renderer_start_editing                    (GtkCellRenderer         *renderer,
                                                                               GdkEvent                *event,
                                                                               GtkWidget               *widget,
                                                                               const gchar             *path,
                                                                               GdkRectangle            *background_area,
                                                                               GdkRectangle            *cell_area,
                                                                               GtkCellRendererState     flags);
static void             fmb_text_renderer_invalidate                       (FmbTextRenderer      *text_renderer);
static void             fmb_text_renderer_set_widget                       (FmbTextRenderer      *text_renderer,
                                                                               GtkWidget               *widget);
static void             fmb_text_renderer_editing_done                     (GtkCellEditable         *editable,
                                                                               FmbTextRenderer      *text_renderer);
static void             fmb_text_renderer_grab_focus                       (GtkWidget               *entry,
                                                                               FmbTextRenderer      *text_renderer);
static gboolean         fmb_text_renderer_focus_out_event                  (GtkWidget               *entry,
                                                                               GdkEventFocus           *event,
                                                                               FmbTextRenderer      *text_renderer);
static void             fmb_text_renderer_populate_popup                   (GtkEntry                *entry,
                                                                               GtkMenu                 *menu,
                                                                               FmbTextRenderer      *text_renderer);
static void             fmb_text_renderer_popup_unmap                      (GtkMenu                 *menu,
                                                                               FmbTextRenderer      *text_renderer);
static gboolean         fmb_text_renderer_entry_menu_popdown_timer         (gpointer                 user_data);
static void             fmb_text_renderer_entry_menu_popdown_timer_destroy (gpointer                 user_data);



struct _FmbTextRendererClass
{
  GtkCellRendererClass __parent__;

  void (*edited) (FmbTextRenderer *text_renderer,
                  const gchar        *path,
                  const gchar        *text);
};

struct _FmbTextRenderer
{
...
G_DEFINE_TYPE (FmbTextRenderer, fmb_text_renderer, GTK_TYPE_CELL_RENDERER)



static void
fmb_text_renderer_class_init (FmbTextRendererClass *klass)
{
  GtkCellRendererClass *gtkcell_renderer_class;
  GObjectClass         *gobject_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize = fmb_text_renderer_finalize;
  gobject_class->get_property = fmb_text_renderer_get_property;
  gobject_class->set_property = fmb_text_renderer_set_property;

  gtkcell_renderer_class = GTK_CELL_RENDERER_CLASS (klass);
  gtkcell_renderer_class->get_size = fmb_text_renderer_get_size;
  gtkcell_renderer_class->render = fmb_text_renderer_render;
  gtkcell_renderer_class->start_editing = fmb_text_renderer_start_editing;

  /**
   * FmbTextRenderer:alignment:
   *
  ...
  /**
   * FmbTextRenderer::edited:
   * @text_renderer : a #FmbTextRenderer.
  ...
  text_renderer_signals[EDITED] =
    g_signal_new (I_("edited"),
                  ...
                  G_SIGNAL_RUN_LAST,
                  G_STRUCT_OFFSET (FmbTextRendererClass, edited),
                  NULL, NULL,
                  _fmb_marshal_VOID__STRING_STRING,
...
static void
fmb_text_renderer_init (FmbTextRenderer *text_renderer)
{
...
static void
fmb_text_renderer_finalize (GObject *object)
{
  FmbTextRenderer *text_renderer = FMB_TEXT_RENDERER (object);
  ...
  fmb_text_renderer_set_widget (text_renderer, NULL);

  (*G_OBJECT_CLASS (fmb_text_renderer_parent_class)->finalize) (object);
}



static void
fmb_text_renderer_get_property (GObject    *object,
                                   guint       prop_id,
                                   GValue     *value,
                                   GParamSpec *pspec)
{
...
static void
fmb_text_renderer_set_property (GObject      *object,
                                   guint         prop_id,
                                   const GValue *value,
                                   GParamSpec   *pspec)
{
...
static void
fmb_text_renderer_get_size (GtkCellRenderer *renderer,
                               GtkWidget       *widget,
                               GdkRectangle    *cell_area,
                               gint            *x_offset,
                               gint            *y_offset,
                               ...
                               gint            *height)
{
  FmbTextRenderer *text_renderer = FMB_TEXT_RENDERER (renderer);
  gint                text_length;
  gint                text_width;
  gint                text_height;

  /* setup the new widget */
  fmb_text_renderer_set_widget (text_renderer, widget);
...
static void
fmb_text_renderer_render (GtkCellRenderer     *renderer,
                             GdkWindow           *window,
                             GtkWidget           *widget,
                             GdkRectangle        *background_area,
                             GdkRectangle        *cell_area,
                             ...
                             GtkCellRendererState flags)
{
  ...
  if (text_renderer->follow_prelit && (flags & GTK_CELL_RENDERER_PRELIT) != 0)
    pango_layout_set_attributes (text_renderer->layout, fmb_pango_attr_list_underline_single ());
  else
...
fmb_text_renderer_start_editing (GtkCellRenderer     *renderer,
                                    GdkEvent            *event,
                                    GtkWidget           *widget,
                                    const gchar         *path,
                                    GdkRectangle        *background_area,
                                    ...
                                    GtkCellRendererState flags)
{
  ...
  g_object_set_data_full (G_OBJECT (text_renderer->entry), I_("fmb-text-renderer-path"), g_strdup (path), g_free);

  /* connect required signals */
  g_signal_connect (G_OBJECT (text_renderer->entry), "editing-done", G_CALLBACK (fmb_text_renderer_editing_done), text_renderer);
  g_signal_connect_after (G_OBJECT (text_renderer->entry), "grab-focus", G_CALLBACK (fmb_text_renderer_grab_focus), text_renderer);
  g_signal_connect (G_OBJECT (text_renderer->entry), "focus-out-event", G_CALLBACK (fmb_text_renderer_focus_out_event), text_renderer);
  g_signal_connect (G_OBJECT (text_renderer->entry), "populate-popup", G_CALLBACK (fmb_text_renderer_populate_popup), text_renderer);
...
static void
fmb_text_renderer_invalidate (FmbTextRenderer *text_renderer)
{
  fmb_text_renderer_set_widget (text_renderer, NULL);
}
...
static void
fmb_text_renderer_set_widget (FmbTextRenderer *text_renderer,
                                 GtkWidget          *widget)
{
  ...
  if (G_UNLIKELY (text_renderer->widget != NULL))
    {
      g_signal_handlers_disconnect_by_func (G_OBJECT (text_renderer->widget), fmb_text_renderer_invalidate, text_renderer);
  ...
  if (G_LIKELY (widget != NULL))
    {
      ...
      g_signal_connect_swapped (G_OBJECT (text_renderer->widget), "destroy", G_CALLBACK (fmb_text_renderer_invalidate), text_renderer);
      g_signal_connect_swapped (G_OBJECT (text_renderer->widget), "style-set", G_CALLBACK (fmb_text_renderer_invalidate), text_renderer);
...
static void
fmb_text_renderer_editing_done (GtkCellEditable    *editable,
                                   FmbTextRenderer *text_renderer)
{
  const gchar *path;
  const gchar *text;
  ...
  g_signal_handlers_disconnect_by_func (G_OBJECT (editable), fmb_text_renderer_editing_done, text_renderer);
  g_signal_handlers_disconnect_by_func (G_OBJECT (editable), fmb_text_renderer_focus_out_event, text_renderer);
  g_signal_handlers_disconnect_by_func (G_OBJECT (editable), fmb_text_renderer_populate_popup, text_renderer);
  ...
  if (G_LIKELY (!GTK_ENTRY (editable)->editing_canceled))
    {
      ...
      path = g_object_get_data (G_OBJECT (editable), "fmb-text-renderer-path");
...
static void
fmb_text_renderer_grab_focus (GtkWidget          *entry,
                                 FmbTextRenderer *text_renderer)
{
  const gchar *text;
  const gchar *dot;
  ...
  dot = fmb_util_str_get_extension (text);
  ...
  g_signal_handlers_disconnect_by_func (G_OBJECT (entry), fmb_text_renderer_grab_focus, text_renderer);
}



static gboolean
fmb_text_renderer_focus_out_event (GtkWidget          *entry,
                                      GdkEventFocus      *event,
                                      FmbTextRenderer *text_renderer)
{
  /* cancel editing if we haven't popped up the menu */
  if (G_LIKELY (!text_renderer->entry_menu_active))
    fmb_text_renderer_editing_done (GTK_CELL_EDITABLE (entry), text_renderer);
...
static void
fmb_text_renderer_populate_popup (GtkEntry           *entry,
                                     GtkMenu            *menu,
                                     FmbTextRenderer *text_renderer)
{
  ...
  g_signal_connect (G_OBJECT (menu), "unmap", G_CALLBACK (fmb_text_renderer_popup_unmap), text_renderer);
}



static void
fmb_text_renderer_popup_unmap (GtkMenu            *menu,
                                  FmbTextRenderer *text_renderer)
{
  ...
  if (G_LIKELY (text_renderer->entry_menu_popdown_timer_id == 0))
    {
      text_renderer->entry_menu_popdown_timer_id = g_timeout_add_full (G_PRIORITY_LOW, 500u, fmb_text_renderer_entry_menu_popdown_timer,
                                                                       text_renderer, fmb_text_renderer_entry_menu_popdown_timer_destroy);
    }
}
...
static gboolean
fmb_text_renderer_entry_menu_popdown_timer (gpointer user_data)
{
  FmbTextRenderer *text_renderer = FMB_TEXT_RENDERER (user_data);

  GDK_THREADS_ENTER ();

  /* check if we still have the keyboard focus */
  if (G_UNLIKELY (!gtk_widget_has_focus (text_renderer->entry)))
    fmb_text_renderer_editing_done (GTK_CELL_EDITABLE (text_renderer->entry), text_renderer);
...
static void
fmb_text_renderer_entry_menu_popdown_timer_destroy (gpointer user_data)
{
  FMB_TEXT_RENDERER (user_data)->entry_menu_popdown_timer_id = 0;
}



/**
 * fmb_text_renderer_new:
 **/
GtkCellRenderer*
fmb_text_renderer_new (void)
{
  return g_object_new (FMB_TYPE_TEXT_RENDERER, NULL);
}
