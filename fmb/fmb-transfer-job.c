...
typedef struct _FmbTransferNode FmbTransferNode;



static void     fmb_transfer_job_get_property (GObject    *object,
                                                  guint       prop_id,
                                                  GValue     *value,
                                                  GParamSpec *pspec);

static void     fmb_transfer_job_set_property (GObject      *object,
                                                  guint         prop_id,
                                                  const GValue *value,
                                                  GParamSpec   *pspec);

static void     fmb_transfer_job_finalize     (GObject                *object);
static gboolean fmb_transfer_job_execute      (BlxoJob                 *job,
                                                  GError                **error);
static void     fmb_transfer_node_free        (gpointer                data);



struct _FmbTransferJobClass
{
  FmbJobClass __parent__;
};

struct _FmbTransferJob
{
  FmbJob             __parent__;

  FmbTransferJobType type;
...
struct _FmbTransferNode
{
  FmbTransferNode *next;
  FmbTransferNode *children;
  GFile              *source_file;
};



G_DEFINE_TYPE (FmbTransferJob, fmb_transfer_job, FMB_TYPE_JOB)



static void
fmb_transfer_job_class_init (FmbTransferJobClass *klass)
{
  GObjectClass *gobject_class;
  BlxoJobClass  *blxojob_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize = fmb_transfer_job_finalize;
  gobject_class->get_property = fmb_transfer_job_get_property;
  gobject_class->set_property = fmb_transfer_job_set_property;

  blxojob_class = BLXO_JOB_CLASS (klass);
  blxojob_class->execute = fmb_transfer_job_execute;
...
static void
fmb_transfer_job_init (FmbTransferJob *job)
{
  job->preferences = fmb_preferences_get ();
  blxo_binding_new (G_OBJECT (job->preferences), "misc-file-size-binary",
...
static void
fmb_transfer_job_finalize (GObject *object)
{
  FmbTransferJob *job = FMB_TRANSFER_JOB (object);

  g_list_free_full (job->source_node_list, fmb_transfer_node_free);

  fmb_g_file_list_free (job->target_file_list);

  g_object_unref (job->preferences);

  (*G_OBJECT_CLASS (fmb_transfer_job_parent_class)->finalize) (object);
}



static void
fmb_transfer_job_get_property (GObject     *object,
                                  guint        prop_id,
                                  GValue      *value,
                                  GParamSpec  *pspec)
{
...
static void
fmb_transfer_job_set_property (GObject      *object,
                                  guint         prop_id,
                                  const GValue *value,
                                  GParamSpec   *pspec)
{
...
static void
fmb_transfer_job_progress (goffset  current_num_bytes,
                              goffset  total_num_bytes,
                              gpointer user_data)
{
  FmbTransferJob *job = user_data;
  guint64            new_percentage;
  gint64             current_time;
  gint64             expired_time;
  guint64            transfer_rate;

  _fmb_return_if_fail (FMB_IS_TRANSFER_JOB (job));
      ...
      if (expired_time > (500 * 1000))
        {
          ...
          blxo_job_percent (BLXO_JOB (job), new_percentage);
...
static gboolean
fmb_transfer_job_collect_node (FmbTransferJob  *job,
                                  FmbTransferNode *node,
                                  GError            **error)
{
  FmbTransferNode *child_node;
  GFileInfo          *info;
  GError             *err = NULL;
  GList              *file_list;
  GList              *lp;

  _fmb_return_val_if_fail (FMB_IS_TRANSFER_JOB (job), FALSE);
  _fmb_return_val_if_fail (node != NULL && G_IS_FILE (node->source_file), FALSE);
  ...
  if (g_file_info_get_file_type (info) == G_FILE_TYPE_DIRECTORY)
    {
      ...
      file_list = fmb_io_scan_directory (FMB_JOB (job), node->source_file,
                                            G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS,
      ...
      for (lp = file_list; err == NULL && lp != NULL; lp = lp->next)
        {
          ...
          child_node = g_slice_new0 (FmbTransferNode);
          ...
          fmb_transfer_job_collect_node (job, child_node, &err);
        }
...
static gboolean
ttj_copy_file (FmbTransferJob *job,
               GFile             *source_file,
               GFile             *target_file,
               ...
               GError           **error)
{
  ...
  _fmb_return_val_if_fail (FMB_IS_TRANSFER_JOB (job), FALSE);
  _fmb_return_val_if_fail (G_IS_FILE (source_file), FALSE);
  _fmb_return_val_if_fail (G_IS_FILE (target_file), FALSE);
  _fmb_return_val_if_fail (error == NULL || *error == NULL, FALSE);
  ...
  if (target_type == G_FILE_TYPE_SYMBOLIC_LINK && (copy_flags & G_FILE_COPY_OVERWRITE) != 0)
    {
      ...
      if (!g_file_delete (target_file, blxo_job_get_cancellable (BLXO_JOB (job)), &err))
        {
          g_propagate_error (error, err);
          return FALSE;
        }
  ...
  g_file_copy (source_file, target_file, copy_flags,
               blxo_job_get_cancellable (BLXO_JOB (job)),
               fmb_transfer_job_progress, job, &err);
...
/**
 * fmb_transfer_job_copy_file:
 * @job                : a #FmbTransferJob.
 ...
 **/
static GFile *
fmb_transfer_job_copy_file (FmbTransferJob *job,
                               GFile             *source_file,
                               GFile             *target_file,
                               GError           **error)
{
  ...
  _fmb_return_val_if_fail (FMB_IS_TRANSFER_JOB (job), NULL);
  ...
  while (err == NULL)
    {
      if (G_LIKELY (!g_file_equal (source_file, target_file)))
      ...
      else
        {
          for (n = 1; err == NULL; ++n)
            {
              GFile *duplicate_file = fmb_io_jobs_util_next_duplicate_file (FMB_JOB (job),
                                                                               source_file,
                                                                               TRUE, n,
                                                                               &err);
      ...
      if (err->domain == G_IO_ERROR && err->code == G_IO_ERROR_EXISTS)
        {
          ...
          response = fmb_job_ask_replace (FMB_JOB (job), source_file,
                                             target_file, &err);
...
static void
fmb_transfer_job_copy_node (FmbTransferJob  *job,
                               FmbTransferNode *node,
                               GFile              *target_file,
                               ...
                               GError            **error)
{
  ...
  _fmb_return_if_fail (FMB_IS_TRANSFER_JOB (job));
  _fmb_return_if_fail (node != NULL && G_IS_FILE (node->source_file));
  _fmb_return_if_fail (target_file == NULL || node->next == NULL);
  _fmb_return_if_fail ((target_file == NULL && target_parent_file != NULL) || (target_file != NULL && target_parent_file == NULL));
  _fmb_return_if_fail (error == NULL || *error == NULL);
  ...
  for (; err == NULL && node != NULL; node = node->next)
    {
      ...
      blxo_job_info_message (BLXO_JOB (job), "%s", g_file_info_get_display_name (info));

retry_copy:
      /* copy the item specified by this node (not recursively) */
      real_target_file = fmb_transfer_job_copy_file (job, node->source_file,
                                                        target_file, &err);
          ...
          if (G_LIKELY (node->source_file != real_target_file))
            {
              ...
              fmb_thumbnail_cache_copy_file (thumbnail_cache,
                                                node->source_file,
                                                real_target_file);
              ...
              if (node->children != NULL)
                {
                  ...
                  fmb_transfer_job_copy_node (job, node->children, NULL, real_target_file, NULL, &err);

                  /* free resources allocted for the children */
                  fmb_transfer_node_free (node->children);
                  node->children = NULL;
                }
              ...
              if (G_LIKELY (target_file_list_return != NULL))
                {
                  *target_file_list_return =
                    fmb_g_file_list_prepend (*target_file_list_return,
                                                real_target_file);
                }

retry_remove:
              /* try to remove the source directory if we are on copy+remove fallback for move */
              if (job->type == FMB_TRANSFER_JOB_MOVE)
                {
                  if (g_file_delete (node->source_file,
                                     ...
                                     &err))
                    {
                      ...
                      fmb_thumbnail_cache_delete_file (thumbnail_cache,
                                                          node->source_file);
                    }
                  else
                    {
                      /* ask the user to retry */
                      response = fmb_job_ask_skip (FMB_JOB (job), "%s",
                                                      err->message);
                      ...
                      if (G_UNLIKELY (response == FMB_JOB_RESPONSE_RETRY))
                        goto retry_remove;
                    }
                }
            }
          ...
          if (err->domain != G_IO_ERROR || err->code != G_IO_ERROR_NO_SPACE)
            {
              ...
              response = fmb_job_ask_skip (FMB_JOB (job), "%s", err->message);
...
static gboolean
fmb_transfer_job_verify_destination (FmbTransferJob  *transfer_job,
                                        GError            **error)
{
  ...
  _fmb_return_val_if_fail (FMB_IS_TRANSFER_JOB (transfer_job), FALSE);
  ...
  /* for all actions in fmb use the same target directory so
  ...
  filesystem_info = g_file_query_filesystem_info (dest, FMBX_FILESYSTEM_INFO_NAMESPACE,
                                                  blxo_job_get_cancellable (BLXO_JOB (transfer_job)),
                                                  NULL);
      ...
      if (transfer_job->total_size > free_space)
        {
          ...
          succeed = fmb_job_ask_no_size (FMB_JOB (transfer_job),
...
static gboolean
fmb_transfer_job_execute (BlxoJob  *job,
                             GError **error)
{
  FmbThumbnailCache *thumbnail_cache;
  FmbTransferNode   *node;
  FmbApplication    *application;
  FmbJobResponse     response;
  FmbTransferJob    *transfer_job = FMB_TRANSFER_JOB (job);
  GFileInfo            *info;
  ...
  blxo_job_info_message (job, _("Collecting files..."));
  ...
  for (sp = transfer_job->source_node_list, tp = transfer_job->target_file_list;
       ...
       sp = snext, tp = tnext)
    {
      ...
      if (transfer_job->type == FMB_TRANSFER_JOB_MOVE
          && fmb_g_file_is_trashed (node->source_file))
        {
          ...
          /* update progress information */
          blxo_job_info_message (job, _("Trying to restore \"%s\""),
          ...
          if (target_parent != NULL)
            parent_exists = g_file_query_exists (target_parent, blxo_job_get_cancellable (job));
          else
            parent_exists = FALSE;

          /* abort on cancellation */
          if (blxo_job_set_error_if_cancelled (job, &err))
            {
          ...
          if (target_parent != NULL && !parent_exists)
            {
              ...
              response = fmb_job_ask_create (FMB_JOB (job),
      ...
      if (transfer_job->type == FMB_TRANSFER_JOB_MOVE)
        {
          /* update progress information */
          blxo_job_info_message (job, _("Trying to move \"%s\""),
                                g_file_info_get_display_name (info));
          ...
          if (g_file_move (node->source_file, tp->data,
                           ...
                           NULL, NULL, &err))
            {
              ...
              fmb_thumbnail_cache_move_file (thumbnail_cache,
                                                node->source_file,
                                                tp->data);

              /* add the target file to the new files list */
              new_files_list = fmb_g_file_list_prepend (new_files_list, tp->data);

              /* release source and target files */
              fmb_transfer_node_free (node);
          ...
          else if (!blxo_job_is_cancelled (job))
            {
              ...
              /* update progress information */
              blxo_job_info_message (job, _("Could not move \"%s\" directly. "
                                           "Collecting files for copying..."),
                                    g_file_info_get_display_name (info));

              if (!fmb_transfer_job_collect_node (transfer_job, node, &err))
                {
      ...
      else if (transfer_job->type == FMB_TRANSFER_JOB_COPY)
        {
          if (!fmb_transfer_job_collect_node (FMB_TRANSFER_JOB (job), node, &err))
            break;
        }
  ...
  if (G_LIKELY (err == NULL))
    {
      /* check destination */
      if (!fmb_transfer_job_verify_destination (transfer_job, &err))
        {
          if (err != NULL)
            {
              g_propagate_error (error, err);
      ...
      for (sp = transfer_job->source_node_list, tp = transfer_job->target_file_list;
        ...
        {
          fmb_transfer_job_copy_node (transfer_job, sp->data, tp->data, NULL,
                                         &new_files_list, &err);
        }
    }
...
static void
fmb_transfer_node_free (gpointer data)
{
  FmbTransferNode *node = data;
  FmbTransferNode *next;
  ...
  while (node != NULL)
    {
      ...
      g_slice_free (FmbTransferNode, node);
...
FmbJob *
fmb_transfer_job_new (GList                *source_node_list,
                         GList                *target_file_list,
                         FmbTransferJobType type)
{
  FmbTransferNode *node;
  ...
  _fmb_return_val_if_fail (source_node_list != NULL, NULL);
  _fmb_return_val_if_fail (target_file_list != NULL, NULL);
  _fmb_return_val_if_fail (g_list_length (source_node_list) == g_list_length (target_file_list), NULL);

  job = g_object_new (FMB_TYPE_TRANSFER_JOB, NULL);
  job->type = type;
  ...
  for (sp = source_node_list, tp = target_file_list;
       ...
       sp = sp->next, tp = tp->next)
    {
      ...
      if (G_UNLIKELY (fmb_g_file_is_root (sp->data) || fmb_g_file_is_root (tp->data)))
        continue;

      /* only process non-equal pairs unless we're copying */
      if (G_LIKELY (type != FMB_TRANSFER_JOB_MOVE || !g_file_equal (sp->data, tp->data)))
        {
          /* append transfer node for this source file */
          node = g_slice_new0 (FmbTransferNode);
          node->source_file = g_object_ref (sp->data);
          job->source_node_list = g_list_append (job->source_node_list, node);

          /* append target file */
          job->target_file_list = fmb_g_file_list_append (job->target_file_list, tp->data);
        }
    }

  /* make sure we didn't mess things up */
  _fmb_assert (g_list_length (job->source_node_list) == g_list_length (job->target_file_list));
...
gchar *
fmb_transfer_job_get_status (FmbTransferJob *job)
{
