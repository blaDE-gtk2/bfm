 ...
 *
 * Tries to invoke the BulkRename() method on a running Fmb instance, that is
 ...
 **/
gboolean
fmb_dbus_client_bulk_rename (const gchar *working_directory,
                                gchar      **filenames,
                                gboolean     standalone,
                                GdkScreen   *screen,
                                ...
                                GError     **error)
{
  ...
  _fmb_return_val_if_fail (g_path_is_absolute (working_directory), FALSE);
  ...
  message = dbus_message_new_method_call ("org.blade.Fmb", "/org/blade/FileManager", "org.blade.Fmb", "BulkRename");
 ...
 * @error             : return location for errors or %NULL.
 *
 * Tries to invoke the LaunchFiles() method on a running Fmb instance, that is
 * registered with the current D-BUS session bus. Returns %TRUE if the method was
 * successfully invoked, else %FALSE.
 *
 * If %TRUE is returned, the current process may afterwards just terminate, as
 ...
 **/
gboolean
fmb_dbus_client_launch_files (const gchar *working_directory,
                                 gchar      **filenames,
                                 GdkScreen   *screen,
                                 const gchar *startup_id,
                                 GError     **error)
{
  ...
  _fmb_return_val_if_fail (g_path_is_absolute (working_directory), FALSE);
  _fmb_return_val_if_fail (filenames != NULL && *filenames != NULL, FALSE);
  _fmb_return_val_if_fail (screen == NULL || GDK_IS_SCREEN (screen), FALSE);
  _fmb_return_val_if_fail (error == NULL || *error == NULL, FALSE);
  ...
  message = dbus_message_new_method_call ("org.blade.Fmb", "/org/blade/FileManager", "org.blade.FileManager", "LaunchFiles");
  dbus_message_set_auto_start (message, FALSE);
  dbus_message_append_args (message,
                            DBUS_TYPE_STRING, &working_directory,
                            DBUS_TYPE_ARRAY, DBUS_TYPE_STRING, &filenames, g_strv_length (filenames),
...
/**
 * fmb_dbus_client_terminate:
 * @error : Return location for errors or %NULL.
 *
 * Tells a running Fmb instance, connected to the D-BUS
 ...
 **/
gboolean
fmb_dbus_client_terminate (GError **error)
{
  DBusConnection *connection;
  DBusMessage    *message;
  DBusMessage    *result;
  ...
  /* generate the LaunchFiles() method (disable activation!) */
  message = dbus_message_new_method_call ("org.blade.Fmb", "/org/blade/FileManager", "org.blade.Fmb", "Terminate");
  dbus_message_set_auto_start (message, FALSE);
