...
fmb_gdk_cairo_create_surface (const GdkPixbuf *pixbuf)
{
  gint             width;
  gint             height;
  guchar          *gdk_pixels;
  ...
  _fmb_return_val_if_fail (GDK_IS_PIXBUF (pixbuf), NULL);
 ...
 **/
GdkScreen*
fmb_gdk_screen_open (const gchar *display_name,
                        GError     **error)
{
  ...
  _fmb_return_val_if_fail (display_name != NULL, NULL);
...
/**
 * fmb_gdk_cairo_set_source_pixbuf:
 * cr       : a Cairo context
 * pixbuf   : a GdkPixbuf
 ...
 * on the pixbuf, which is efficient within Fmb because we also share
 * the pixbufs using the icon cache.
 **/
void
fmb_gdk_cairo_set_source_pixbuf (cairo_t   *cr,
                                    GdkPixbuf *pixbuf,
                                    gdouble    pixbuf_x,
                                    gdouble    pixbuf_y)
{
  ...
  if (G_UNLIKELY (surface_quark == 0))
    surface_quark = g_quark_from_static_string ("fmb-gdk-surface");
  ...
  if (surface == NULL)
    {
      ...
      surface = fmb_gdk_cairo_create_surface (pixbuf);
